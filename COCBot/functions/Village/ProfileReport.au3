...
Func ProfileReport()
	...
	Local $CheckTopProfile[4] = [200, 166, 0x868CAC, 5]
	Local $CheckTopProfile2[4] = [220, 355, 0x4E4D79, 5]
	For $i = 0 to 1 ; Check twice,  because the button is animated
		If QuickMIS("BC1", $sClaimDirectory, 680, 165, 855, 680) Then
			...
			For $i = 0 To 9
				...
				If _ColorCheck(_GetPixelColor($CheckTopProfile[0], $CheckTopProfile[1], True), Hex($CheckTopProfile[2], 6), $CheckTopProfile[3])= True _
				  And _ColorCheck(_GetPixelColor($CheckTopProfile2[0], $CheckTopProfile2[1], True), Hex($CheckTopProfile2[2], 6), $CheckTopProfile2[3]) = True Then ExitLoop
			Next
