...
if %compile_only% NEQ true (
	...
	echo Creating %zip%
	"build\7z.exe" a -t7z -mx9 -x!*.7z -x!*.zip -xr!BackUp -x!.git* -x!Profiles "-x!OLD CODE" -x!build.bat -x!build -x!build\* -x!Zombies -x!Zombies\* -x!SkippedZombies -x!SkippedZombies\* -x!lib\ImgLocDebugData -x!lib\ImgLocDebugData\* -x!*debug.txt -x!lib\imglocdebug.txt -r "%zip%" *
)
pause